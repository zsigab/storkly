...
describe("RegistryPage", () => {
  ...
  it("hides Claim button for authenticated user without RSVP to linked event", async () => {
    const { api } = await import("@/api");
    vi.mocked(api.GET).mockImplementation((path: string) => {
      if (path === "/api/registries/{slug}") {
        return Promise.resolve({
          data: { ...registryFixture, hasLinkedEvent: true, userRsvpedYes: false },
          error: undefined,
          response: new Response(),
        });
      }
      if (path === "/api/registries/{slug}/categories") {
        return Promise.resolve({
          data: [{ id: "cat-1", registryId: "1", name: "Test", sortOrder: 0, isDefault: true }],
          error: undefined,
          response: new Response(),
        });
      }
  ...
  it("shows Claim button for authenticated user with RSVP to linked event", async () => {
    const { api } = await import("@/api");
    vi.mocked(api.GET).mockImplementation((path: string) => {
      if (path === "/api/registries/{slug}") {
        return Promise.resolve({
          data: { ...registryFixture, hasLinkedEvent: true, userRsvpedYes: true },
          error: undefined,
          response: new Response(),
        });
      }
  ...
  it("shows Claim button for authenticated user when registry has no linked event", async () => {
    const { api } = await import("@/api");
    vi.mocked(api.GET).mockImplementation((path: string) => {
      if (path === "/api/registries/{slug}") {
        return Promise.resolve({
          data: { ...registryFixture, hasLinkedEvent: false, userRsvpedYes: null },
          error: undefined,
          response: new Response(),
        });
      }
